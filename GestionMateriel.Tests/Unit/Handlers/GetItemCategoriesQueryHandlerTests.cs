...
public class GetItemCategoriesQueryHandlerTests
{
    ...
    public async Task Handle_Should_Return_Mapped_Categories_By_Structure()
    {
        ...
        repoMock.Setup(r => r.GetByStructureAsync(1)).ReturnsAsync(new List<ItemCategory>
        {
        ...
        var result = (await handler.Handle(new GetItemCategoriesQuery(1), CancellationToken.None)).ToList();
