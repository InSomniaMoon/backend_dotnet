...
{
    [Fact]
    public async Task Handle_Should_Throw_KeyNotFoundException_When_Structure_Not_Found()
    {
        await using var db = TestHelper.CreateDbContext();
        var handler = new GetStructureMembersQueryHandler(db);
        await Assert.ThrowsAsync<KeyNotFoundException>(() => handler.Handle(new GetStructureMembersQuery(99), CancellationToken.None));
    }
