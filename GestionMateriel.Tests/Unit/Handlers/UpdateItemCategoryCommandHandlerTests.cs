...
public class UpdateItemCategoryCommandHandlerTests
{
    ...
    public async Task Handle_Should_Update_Category()
    {
        ...
        db.ItemCategories.Add(new ItemCategory { Id = 1, Name = "Old", Identified = false, StructureId = 1 });
        await db.SaveChangesAsync();
        var handler = new UpdateItemCategoryCommandHandler(db, TestHelper.CreateMapper());
        var result = await handler.Handle(new UpdateItemCategoryCommand(1, new UpdateItemCategoryRequest { Name = "New", Identified = true }), CancellationToken.None);
